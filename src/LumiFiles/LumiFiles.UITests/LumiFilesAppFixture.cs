...
namespace LumiFiles.UITests;

/// <summary>
/// Shared fixture that manages FlaUI automation handle to the Lumi Files application.
///
/// WinUI 3 apps require MSIX packaging and cannot be launched via Application.Launch().
/// Tests attach to an already-running Lumi Files process.
///
/// Usage:
///   1. Launch Lumi Files from Visual Studio (F5) or Start Menu
///   2. Run tests: dotnet test src/LumiFiles/LumiFiles.UITests/LumiFiles.UITests.csproj -p:Platform=x64
/// </summary>
public static class SpanAppFixture
{
    ...
    /// <summary>
    /// Attach to the running Lumi Files process and return its main window.
    ...
    public static Window GetMainWindow(int timeoutSeconds = 10)
    {
        ...
        {
            var processes = Process.GetProcessesByName("LumiFiles");
            if (processes.Length == 0)
            {
                throw new InvalidOperationException(
                    "No running Lumi Files process found.\n" +
                    "WinUI 3 apps require MSIX packaging — launch Lumi Files from Visual Studio (F5) or Start Menu first.");
            }
        ...
        if (mainWindow == null)
            throw new TimeoutException($"Lumi Files main window did not appear within {timeoutSeconds}s");
    ...
    /// <summary>
    /// Bring the LumiFiles window to foreground and ensure it has keyboard focus.
    ...
    private static Window? GetMainWindowOrNull()
    {
        try
        {
            ...
            var process = System.Diagnostics.Process.GetProcessesByName("LumiFiles");
